...
if nargin < 1
    ...
    command = sprintf('curl -s -k -u %s:%s -H cipres-appkey:%s "%s/job/%s?expand=true" > tmptxt.xml', nsgusername,  nsgpassword, nsgkey, nsgurl, nsgusername); % request full job status objects
    system(command);
    try
    ...
    if strcmpi(filetype, 'xml')
        command = sprintf(['curl -s -k -u %s:%s -H cipres-appkey:%s "%s?expand=true" > tmptxt.' filetype], nsgusername, nsgpassword, nsgkey, jobname);
        system(command);
        try
            res2 = xml2struct('tmptxt.xml');
            delete('tmptxt.xml');
